...
데이터 엔지니어링 직무에 관심을 가지게 된 이후 10억행 챌린지(1BRC)를 해보면서 단순히 기술들을 사용하여 대용량 데이터처리가 잘 되는지 결과만 확인했어요.

핵심은 단순 결과가 아니라, 설계부터 시작해서 운영까지 책임지며 현재 시스템에 적합한 설정을 찾아가는 과정이 중요하다고 생각되어 역량을 기르기 위한 고민을 했어요.

한발 더 들어가서 데이터 엔지니어의 업무를 깊게 분석해보는 과정에서 스트리밍 데이터를 처리하는 것에 매력을 느끼고 이 시스템을 안정적이고 확장가능한 구조를 설계하고 구축하는 역량을 키워야겠다는 필요성을 느꼈어요.

“스트리밍 데이터를 다루는 비지니스가 뭐가 있을까?” 고민 하던 중 광고 도메인의 노출 대비 클릭의 비율(CTR)의 개념 발견 했어요. 추후 개인화된 광고 서비스에 사용될 수 있을 것 같다는 생각에 흥미를 느껴 주제로 선정했어요.
...
![](/images/posts/실시간-CTR-파이프라인-구축기/c6caa47f9482.png)
...
![](/images/posts/실시간-CTR-파이프라인-구축기/8dd7886a1d3c.png)
...
![](/images/posts/실시간-CTR-파이프라인-구축기/7697dd66f3bb.png)
...
![](/images/posts/실시간-CTR-파이프라인-구축기/a5cac3289908.png)

## 4.1 전체 데이터 흐름

Impressions/Clicks → Kafka → Flink → Redis/ClickHouse/DuckDB → FastAPI

![](/images/posts/실시간-CTR-파이프라인-구축기/99dd367bd82c.png)
...
![](/images/posts/실시간-CTR-파이프라인-구축기/bfc4e38df326.png)
...
![](/images/posts/실시간-CTR-파이프라인-구축기/4a2f8371c29f.png)
