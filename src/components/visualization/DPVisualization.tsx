...
function Cell3D({
    ...
    const color =
        status === 'computed' ? '#6bcf7f' :
            status === 'calculating' ? '#ff6b6b' :
                status === 'using' ? '#ffd93d' :
                    '#4a90e2';
...
function DPScene({
    ...
    return (
        <>
            ...
            {showArrows && arrowFrom.map((fromIdx, i) => {
                if (fromIdx >= 0 && fromIdx < cells.length && arrowTo >= 0 && arrowTo < cells.length) {
                    return (
                        <Arrow
                            key={i}
...
export default function DPVisualization() {
    ...
    const fibonacci = async () => {
        ...
        for (let i = 2; i <= n; i++) {
            ...
            setArrowTo(i);
            setCells([...dp]);
            setMessage(`F(${i}) = F(${i - 1}) + F(${i - 2}) = ${dp[i - 1].value} + ${dp[i - 2].value}`);
                ...
                <div className="space-y-4">
                    <div className="flex flex-wrap gap-4 items-center justify-between">
                        <div className="flex gap-4 items-center">
                            <div className="flex items-center gap-2">
                                <label className="text-white text-sm font-semibold">N 값:</label>
                                <input
                                    type="number"
                                    value={n}
                    ...
                    <div className="bg-purple-500/10 border border-purple-500/30 rounded-lg p-3">
                        <p className="text-purple-300 text-sm">
                            💡 <strong>동적 프로그래밍 (DP):</strong> 작은 부분 문제의 결과를 테이블에 저장하고 재사용하여, 중복 계산을 제거합니다. 피보나치는 O(2ⁿ) → O(n)으로 최적화됩니다.
                        </p>
                    </div>
