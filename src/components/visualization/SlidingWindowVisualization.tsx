...
function Box3DWindow({
    ...
    const color =
        status === 'max-window' ? '#6bcf7f' :
            status === 'in-window' ? '#3498db' :
                status === 'passed' ? '#95a5a6' :
                    '#4a90e2';
...
export default function SlidingWindowVisualization() {
    ...
    const slidingWindow = async () => {
        ...
        for (let i = windowSize; i < arr.length; i++) {
            ...
            setWindowStart(step);
            setElements([...arr]);
            setMessage(`윈도우 [${step}-${i}]: 합 = ${windowSum} (제거: ${arr[i - windowSize].value}, 추가: ${arr[i].value})`);
            await sleep(speed);
            ...
            if (windowSum > currentMax) {
                ...
                setElements([...arr]);
                setMessage(`✨ 새로운 최대값 발견! 윈도우 [${step}-${i}]: 합 = ${windowSum}`);
                await sleep(speed * 1.5);
            }
        }

        setMessage(`✅ 완료! 최대 합: ${currentMax} (윈도우 [${currentMaxStart}-${currentMaxStart + windowSize - 1}])`);
        setIsPlaying(false);
    };
                ...
                <div className="space-y-4">
                    <div className="flex flex-wrap gap-4 items-center justify-between">
                        <div className="flex gap-4 items-center">
                            <div className="flex items-center gap-2">
                                <label className="text-white text-sm font-semibold">윈도우 크기:</label>
                                <input
                                    type="number"
                                    value={windowSize}
                                    onChange={(e) => setWindowSize(Math.max(2, Math.min(5, Number(e.target.value))))}
                                    disabled={isPlaying}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            <div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-3">
                                <p className="text-blue-300 text-sm">
                                    <strong>현재 윈도우:</strong> {windowStart >= 0 ? `[${windowStart}-${windowStart + windowSize - 1}]` : '-'}
                                </p>
                            </div>
                            <div className="bg-green-500/10 border border-green-500/30 rounded-lg p-3">
                                <p className="text-green-300 text-sm">
                                    <strong>최대 합:</strong> {maxSum} (윈도우 [{maxWindowStart}-{maxWindowStart + windowSize - 1}])
                                </p>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="bg-purple-500/10 border border-purple-500/30 rounded-lg p-3">
                        <p className="text-purple-300 text-sm">
                            💡 <strong>슬라이딩 윈도우:</strong> 고정 크기의 윈도우를 한 칸씩 이동하며, 이전 계산 결과를 재사용하여 O(n) 시간에 최대/최소값을 찾습니다.
                        </p>
                    </div>
