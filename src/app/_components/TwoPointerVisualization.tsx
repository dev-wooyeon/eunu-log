...
interface Element {
    value: number;
    status: 'default' | 'left-pointer' | 'right-pointer' | 'both-pointers' | 'found' | 'checked';
}
...
function Box3DWithPointer({
    ...
    useFrame(() => {
        if (meshRef.current && (status === 'left-pointer' || status === 'right-pointer' || status === 'both-pointers')) {
    ...
    const color =
        status === 'found' ? '#6bcf7f' :
            status === 'both-pointers' ? '#e74c3c' :
                status === 'left-pointer' ? '#3498db' :
                    status === 'right-pointer' ? '#f39c12' :
                        status === 'checked' ? '#95a5a6' :
                            '#4a90e2';
...
export default function TwoPointerVisualization() {
    ...
    const twoPointerSearch = async () => {
        ...
        while (left < right) {
            ...
            setMessage(`Step ${steps}: arr[${left}] + arr[${right}] = ${arr[left].value} + ${arr[right].value} = ${sum}`);
            await sleep(speed);
            ...
            if (sum === target) {
                ...
                setElements([...arr]);
                setMessage(`✅ 찾았습니다! [${arr[left].value}, ${arr[right].value}] = ${target} (${steps}번 만에 발견)`);
                setIsPlaying(false);
                return;
            }
                ...
                <div className="space-y-4">
                    <div className="flex flex-wrap gap-4 items-center justify-between">
                        <div className="flex gap-4 items-center">
                            <div className="flex items-center gap-2">
                                <label className="text-white text-sm font-semibold">목표 합:</label>
                                <input
                                    type="number"
                                    value={target}
                    ...
                    <div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-3">
                        <p className="text-blue-300 text-sm">
                            💡 <strong>투 포인터 알고리즘:</strong> 정렬된 배열에서 두 포인터를 양 끝에서 시작하여, 합이 목표값보다 작으면 왼쪽 포인터를 오른쪽으로, 크면 오른쪽 포인터를 왼쪽으로 이동합니다.
                        </p>
                    </div>
