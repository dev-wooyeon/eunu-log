...
export default function BinarySearchVisualization() {
    ...
    const initializeArray = () => {
        ...
        setTarget(newElements[Math.floor(Math.random() * newElements.length)].value);
        setMessage('');
        setIsPlaying(false);
    };
    ...
    const binarySearch = async () => {
        ...
        while (left <= right) {
            ...
            arr[mid].status = 'mid';
            setElements([...arr]);
            setMessage(`Step ${steps}: mid = ${mid}, arr[${mid}] = ${arr[mid].value}, target = ${target}`);
                    ...
                    <div className="flex flex-wrap gap-4 items-center justify-between">
                        <div className="flex gap-4 items-center">
                            <div className="flex items-center gap-2">
                                <label className="text-white text-sm font-semibold">찾을 값:</label>
                                <input
                                    type="number"
                                    value={target}
