...
export default function Resume() {
  return (
    <div className="min-h-screen p-8 bg-primary max-md:p-4 max-[480px]:px-3">
      <header className="max-w-[800px] mx-auto mb-16">
        <BackLink href="/" text="← Home" />
        <h1 className="font-[ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,'Segoe_UI',sans-serif] text-[clamp(2.5rem,6vw,3.5rem)] font-bold text-text-primary m-0 tracking-[-0.02em] max-md:text-[2rem] max-[480px]:text-[1.75rem]">
          Resume
        </h1>
      </header>
      ...
      <main className="max-w-[800px] mx-auto">
        ...
        <section className="grid grid-cols-4 gap-10 mb-16 pb-12 border-b border-border max-lg:grid-cols-2 max-lg:gap-x-8 max-lg:gap-y-6 max-lg:mb-12 max-lg:pb-10">
          <div className="flex flex-col gap-3">
            ...
            <div className="text-[15px] text-text-primary leading-relaxed font-normal max-lg:text-sm max-[480px]:text-[13px]">
              {personalInfo.name}<br />
              <span className="text-[13px] text-text-tertiary font-normal max-lg:text-xs max-[480px]:text-[11px]">{personalInfo.birthDate}</span>
            </div>
          </div>
          <div className="flex flex-col gap-3">
            <div className="text-xs text-text-tertiary uppercase tracking-[0.08em] font-semibold max-lg:text-[11px]">Position / </div>
            <div className="text-[15px] text-text-primary leading-relaxed font-normal max-lg:text-sm max-[480px]:text-[13px]">{personalInfo.position}</div>
          </div>
          <div className="flex flex-col gap-3">
            <div className="text-xs text-text-tertiary uppercase tracking-[0.08em] font-semibold max-lg:text-[11px]">Keyword / </div>
            <div className="text-[15px] text-text-primary leading-relaxed font-normal max-lg:text-sm max-[480px]:text-[13px]">{personalInfo.keywords}</div>
          </div>
          <div className="flex flex-col gap-3">
              ...
              <div className="flex flex-col gap-6">
                {exp.projects.map((project, projIndex) => (
                  <article key={projIndex} className="flex flex-col gap-4 pb-6 border-b border-border last:border-b-0 last:pb-0 max-lg:gap-3 max-lg:pb-6">
                    ...
                    <p className="text-[15px] text-text-secondary leading-[1.7] m-0 font-normal max-lg:text-sm max-[480px]:text-[13px]">
                      {project.description}
                    </p>
                    <ul className="list-disc pl-6 m-0 flex flex-col gap-2">
                      {project.achievements.map((achievement, achIndex) => (
                        <li key={achIndex} className="text-[15px] text-text-secondary leading-[1.7] max-lg:text-sm max-[480px]:text-[13px]">
                          {achievement}
