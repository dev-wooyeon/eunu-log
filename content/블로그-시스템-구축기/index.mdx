...
- 3가지 다른 breakpoint 패턴 혼재 (max-md, max-lg, custom-480)
- 일관성 없는 반응형 처리
...
```diff
- className="text-clamp-hero max-md:text-2xl max-sm:text-xl"
+ className="text-display-md md:text-2xl sm:text-xl"
```
...
export function PageLayout({ title, children }: Props) {
  return (
    <div className="min-h-screen p-8 bg-primary max-md:p-4">
      <header className="max-w-content-md mx-auto pb-8">
        <BackLink href="/" text="← Home" />
        <h1 className="text-display-lg font-bold">{title}</h1>
      </header>
      <main className="max-w-content-md mx-auto">{children}</main>
    </div>
  );
}
```
...
export default function FeedPage() {
  return (
    <div className="min-h-screen p-8 bg-primary max-md:p-4">
      ...
      <main className="max-w-content-md mx-auto">
